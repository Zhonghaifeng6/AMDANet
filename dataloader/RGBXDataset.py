...
class RGBXDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.preprocess is not None and self._split_name == 'train':
            output_dict = dict(data=rgb, label=gt, modal_x=x, Mask=Mask, fn=str(item_name), n=len(self._file_names))
        else:
    ...
    def _open_image(filepath, mode="Gray", dtype=np.float32):
        if mode == "RGB":
            ...
            if img is None:
                raise FileNotFoundError(f"Image not found or cannot be opened: {filepath}")
