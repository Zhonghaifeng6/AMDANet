...
from models.model import AMDANet, group_weight


dataset_name = "PST900"
parser = argparse.ArgumentParser()
parser.add_argument('--continue_fpath', default="", type=str, help='Description of your argument')
# parser.add_argument('--seed', default=11223, type=int, help='set seed everything')
...
parser.add_argument('--x_is_single_channel', default=True, type=bool, help='True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input')
...
parser.add_argument('--class_names',
                    default=['Background', 'Hand-Drill', 'BackPack', 'Fire-Extinguisher', 'Survivor'],
...
parser.add_argument('--lr', default=6.3e-5, type=float, help='learning rate')
...
parser.add_argument('--nepochs', default=300, type=int, help='loop forever, stop manually')
parser.add_argument('--warm_up_epoch', default=75, type=int, help='')
parser.add_argument('--num_workers', default=16, type=int, help='')
parser.add_argument('--train_scale_array', default=[0.5, 0.75,1, 1.25, 1.5, 1.75, 2], type=list, help='')
...
parser.add_argument('--checkpoint_step', default=1, type=int, help='save checkpoint per step')
# Logger Config
...
with Engine(custom_parser=parser) as engine:
    ...
    # model = AMDANet(cfg=args, criterion=criterion, norm_layer=BatchNorm2d, enable_mutual_masking=True)
    model = AMDANet(cfg=args, criterion=criterion, norm_layer=BatchNorm2d)
    base_lr = args.lr
    ...
    for epoch in range(engine.state.epoch, args.nepochs+1):
        ...
        if (epoch >= args.checkpoint_start_epoch) and (epoch % args.checkpoint_step == 0) or (epoch == args.nepochs):
            ...
            for idx in pbar:
                ...
                with torch.no_grad():
                    # model.enable_mutual_masking = False
                    score, _, Fus_img, _ = model(img, modal_x)
                    ...
                    # resize label
                    label_np = cv2.resize(label.squeeze().cpu().numpy(), (args.eval_size[1], args.eval_size[0]), interpolation=cv2.INTER_NEAREST)
                    pred = processed_pred.argmax(2)
                result = hist_info(args.num_classes, pred, label.squeeze().cpu().numpy())[0]
            ...
            if engine.distributed and (engine.local_rank == 0) or (not engine.distributed):
                ...
                if mIoU[0] > best_mIoU:
                    best_mIoU = mIoU[0]
                    engine.save_checkpoint(os.path.join(args.checkpoint_dir, "best" + str(epoch)+ ".pth"))
