...
def print_iou(iou, freq_IoU, mean_pixel_acc, pixel_acc, class_acc, class_names=None, show_no_back=False, no_print=False):
    n = iou.size
    lines = []
    for i in range(n):
    ...
    print(type(mean_IoU), mean_IoU)
    print(type(freq_IoU), freq_IoU)
    if show_no_back:
